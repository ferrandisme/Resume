...
export function formatPublishedDate(date: string, locale: Locale): string {
...
export function sortByOrderDesc<T extends { data: { order: number } }>(
  items: T[],
): T[] {
  return [...items].sort((a, b) => b.data.order - a.data.order);
}

export function sortByYearDesc<T extends { data: { year: number } }>(
  items: T[],
): T[] {
